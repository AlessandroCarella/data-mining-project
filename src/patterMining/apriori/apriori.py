...
allFrequentItemSets = apriori(X, target="s", supp=supp, zmin=zmin,report="S")
print('\nFrequent Item Set\n')
print (pd.DataFrame(allFrequentItemSets, columns=["frequent_itemset", "support"]))
closedFrequentItemSets = apriori(X, target="c", supp=supp, zmin=zmin, report="S")
print('\nClosed Item Set\n')
print (pd.DataFrame(closedFrequentItemSets, columns=["frequent_itemset", "support"]))
maximalFrequentItemSets = apriori(X, target="m", supp=supp, zmin=zmin, report="S")
print('\nMaximal Item Set\n')
...
title='Closed vs. Maximal Itemsets Apriori'
plt.title(title)
plt.legend()
plt.grid(True)
plt.savefig(os.path.join('src/patterMining/apriori/charts', title + ".png"))


conf = 70
rules = apriori(X, target="r", supp=supp, zmin=zmin, conf=conf, report="aScl")
generatedRules = apriori(X, target="g", supp=supp, zmin=zmin, conf=conf, report="aScl")
...
rules_df.sort_values(by="lift", axis=0, ascending=False).to_csv(os.path.join('src/patterMining/apriori/results',csv_filename), index=False)
#classification rules for "explicit" attribute when value is False
rules_df[rules_df["consequent"] == "notExplicit"].sort_values(by="lift", ascending=False).to_csv(os.path.join('src/patterMining/apriori/results',csv_filename_classification), index=False)