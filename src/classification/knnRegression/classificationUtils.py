...
def getModelPath (modelName:str):
    ...
    return path.join(getModelsPickleFolder(), "Knn Reg Model.pickle")
...
def saveModelToPickleFile (model):
    with open (createModelPath ("knnReg"), "wb") as f:
        pickle.dump (model, f)
...
def saveOtherInfoModelDict (modelDict:dict):
    ...
    with open (getInfoPath("knnReg"), "w") as file:
        file.write ("knn regressor dict infos:\n")
        file.write("\n---------------------\n")

    for key, values in modelDict.items():
        addSingleInstanceToFile (getInfoPath ("knnReg"), key, {
                "k":values.get("k"),
