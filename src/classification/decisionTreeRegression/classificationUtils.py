...
def getModelPath (modelName:str):
    ...
    return path.join(getModelsPickleFolder(), "Decision Tree Reg Model.pickle")
...
def saveModelToPickleFile (model):
    with open (createModelPath ("decisionTreeReg"), "wb") as f:
        pickle.dump (model, f)
...
def saveOtherInfoModelDict (modelDict:dict):
    ...
    with open (getInfoPath("decisionTreeReg"), "w") as file:
        file.write ("decision treee regressor dict infos:\n")
        file.write("\n---------------------\n")

    for key, values in modelDict.items():
        addSingleInstanceToFile (getInfoPath ("decisionTreeReg"), key, {
                "k":values.get("k"),
